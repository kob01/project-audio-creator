    ...
    <Drawer anchor='left' open={Imitation.state.navigationDrawerExpand} onClose={() => Imitation.assignState({ navigationDrawerExpand: false })} sx={{ '& .MuiPaper-root': { width: 304, height: '100%' } }} >
      <div style={{ height: '100%', padding: 16, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflow: 'auto' }}>
        <div>
          ...
          <Button style={{ marginBottom: 4, justifyContent: 'start' }} fullWidth variant={pathname === '/PianoV1M2' ? 'contained' : 'outlined'} onClick={() => { push('/PianoV1M2'); Imitation.assignState({ navigationDrawerExpand: false }); }}><MusicNoteIcon style={{ marginRight: 4 }} />Piano V1 M2</Button>
        ...
        <div>
          <Button style={{ marginTop: 4, justifyContent: 'start' }} fullWidth variant={pathname === '/Record' ? 'contained' : 'outlined'} onClick={() => { push('/Record'); Imitation.assignState({ navigationDrawerExpand: false }); }}><MicIcon style={{ marginRight: 4 }} />Record</Button>
