  ...
  return <>
    ...
    <Drawer anchor='left' open={drawer} onClose={() => setDrawer(false)} sx={{ '& .MuiPaper-root': { width: 304, height: '100%' } }} >
      <div style={{ height: '100%', padding: 16, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', overflow: 'auto' }}>
        <div>
          <Button style={{ margin: '4px 0', justifyContent: 'start' }} fullWidth variant={pathname === '/SimplePiano' ? 'contained' : 'outlined'} onClick={() => { push('/SimplePiano'); setDrawer(false); }}><ArticleIcon style={{ marginRight: 4 }} />Simple Piano</Button>
          <Button style={{ margin: '4px 0', justifyContent: 'start' }} fullWidth variant={pathname === '/BassoonStacF1' ? 'contained' : 'outlined'} onClick={() => { push('/BassoonStacF1'); setDrawer(false); }}><ArticleIcon style={{ marginRight: 4 }} />Bassoon StacF1</Button>
        </div>
