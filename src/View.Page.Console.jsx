    ...
    <div style={{ height: '100%', flexGrow: 0, flexShrink: 0, display: 'flex', flexDirection: 'column', overflow: 'auto' }}>
      {
        Imitation.state.console.map((i, index) => {
          return <Animation tag={Button} restore={true} animation={[{ opacity: 0 }, { opacity: 1 }]} key={i.id} style={{ margin: '4px 0', textAlign: 'left', transition: '0.5s all' }} fullWidth variant={i === Imitation.state.consoleCurrent ? 'contained' : 'outlined'} onClick={() => Imitation.setState(pre => { pre.consoleCurrent = i; return pre })}>{i.name}</Animation>
        })
      }
    </div>
        ...
        <div style={{ width: '100%', height: '100%', position: 'absolute', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {
            new Array(101).fill().map((i, index) => {
              if (index % 10 === 0) {
                return <div key={index} style={{ width: 2, height: 8, background: Imitation.state.theme.palette.primary.main, transition: '0.5s all' }}></div>
              }
              if (index % 5 === 0) {
                return <div key={index} style={{ width: 2, height: 6, background: Imitation.state.theme.palette.primary.main, transition: '0.5s all' }}></div>
              }
          ...
          <div style={{ width: '100%', height: '100%', position: 'absolute', overflow: 'auto' }}>
            {
              Imitation.state.consoleCurrent.group.map((i, index) => {
                return <ConsoleSource key={index} source={i} style={{ width: `${i.duration / maxTime * 100}%`, height: 40, top: index * 48, left: `${i.when / maxTime * 100}%` }} />
              })
            }
          </div>
          : null
